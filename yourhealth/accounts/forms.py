...
class ChangeProfileForm(forms.ModelForm):
    ...
    def clean_phone_number(self):
        ...
        user = User.objects.filter(Q(phone_number__iexact=phone_number) & ~Q(id=self.user.id)).exists()
        if user:
            raise ValidationError(_('You can not use this phone number.'))
...
class AdditionalInfoForm(forms.ModelForm):
    gender = forms.ChoiceField(widget=forms.RadioSelect, choices=Profile.GENDER_CHOICES)
